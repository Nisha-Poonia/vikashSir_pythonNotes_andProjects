...
class Customer:
    ...
    def searchCustomer(self): #self=8000, self.id=20
        ...
        #         self.mob=e.mob
        #         return 1
        Customer.cur.execute(f"select * from custb where id='{self.id}'")
    ...
    @staticmethod
    def createSchema():
        Customer.cur.execute("create database cusdb")
        Customer.cur.execute("use cusdb")
        Customer.cur.execute("create table custb (id varchar(5),name varchar(30),age varchar(5),mob varchar(15))")
    @staticmethod
    def selectDB():
        Customer.cur.execute("use cusdb")
...
while(1):
    ...
    if(choice=="1"): #Add Customer
        ...
        Customer.con.close()
        print("Thanks for Using Akhilesh's CMS")
        break
